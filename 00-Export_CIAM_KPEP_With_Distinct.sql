WITH union_evt AS (
  ...
  WHERE usr.realm_id != 'master'
    AND TO_TIMESTAMP(usr.created_timestamp / 1000)::date BETWEEN TO_TIMESTAMP('2000-03-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('2025-11-30 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
    AND att.value IN (
      __LISTE_IDS__
      )
  ...
  WHERE usr.realm_id != 'master'
    ...
    AND TO_TIMESTAMP(evt.event_time / 1000)::date BETWEEN TO_TIMESTAMP('2000-03-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS') AND TO_TIMESTAMP('2025-11-30 00:00:00', 'YYYY-MM-DD HH24:MI:SS')
    AND att.value IN (
      __LISTE_IDS__
      )
)
...
FROM (
  SELECT *,
         ROW_NUMBER() OVER (PARTITION BY idkpep ORDER BY date_evt DESC, heure_evt DESC) AS rang
  FROM union_evt
) final
WHERE rang = 1;
