...
"""
NSAC Quality Pipeline v4 — KPI intégrés (5 cartes + 5 graphes), scan multi-colonnes e-mail,
présence CIAM/IEHE, Assurés/Assurés%, doublons, et rendu HTML modernisé.

Nouveautés:
- Lecture résiliente de KPI.csv (texte libre + table code_soc/motif/count) pour:
  * Cartes: Contrats (KPI), Résiliations totales (KPI), Total anomalies (KPI + répartition).
  * Graphes: Déphasages (camembert), Volumétrie contrats (camembert),
             Répartition par motif (barres), Répartition par code_soc 010/044/052 (barres).
- “Mismatch NS↔IEHE” = couverture idrpp (numpersonne) de NEW_S dans IEHE.
- “Mismatch NS↔CIAM” = couverture e-mail de NEW_S (toutes colonnes) dans CM.
- “Assurés” = N porteurs (ASSPRI*). “Assurés%” = ratio porteurs/NEW_S.
- “E-mails invalides / jetables” + “Valides (regex)” scannent toutes les colonnes e-mail.
- Tableau “TP & délais” SUPPRIMÉ (les cartes TP restent si vous les utilisez).
- Graphiques 3 par ligne (min 320px), légendes avec nombres pour corréler avec les %.
"""
...
def read_csv_smart(path, purpose=None):
    """Robust CSV reader with smart sep/encoding detection and defensive options."""
    import pandas as pd
    if not path:
...
def write_csv(df: pd.DataFrame, path: Path) -> None:
    ...
    try:
        ...
        df.to_csv(path, index=False, sep=";", encoding="cp1252")
...
def compute_payload(NS: pd.DataFrame,
    ...
    tp_total = tp_ok + tp_negative
    tp_total_pct = pct(tp_total, tp_expected if tp_expected else 0)
    ...
    kpi = {
        ...
        "tp_ok_pct": as_pct_str(tp_pct), "tp_ok_badge_cls": tp_badge_cls, "tp_ok_badge": tp_badge,
        "tp_ok": f"{tp_ok}", "tp_expected": f"{tp_expected}",
        "tp_future_pct": as_pct_str(tp_future_pct), "tp_future": f"{tp_future}",
        "tp_negative_pct": as_pct_str(tp_negative_pct), "tp_negative": f"{tp_negative}", "tp_total_pct": as_pct_str(tp_total_pct), "tp_total": f"{tp_total}",
        ...
        "tp_window": f"{args.tp_window_days}", "tp_window_plus1": f"{args.tp_window_days + 1}",
    }
...
def main():
    ...
    ap.add_argument("--console-verbose", action="store_true", help="Affiche tous les KPI et diagnostics dans le terminal.")
    ap.add_argument("--no-html", action="store_true", help="Ne pas générer le rapport HTML.")
    ...
    print("[INFO] ╔═══════════════════════════════════════╗")
    print("[INFO] ║  NSAC Quality Pipeline v4             ║")
    print("[INFO] ║  Génération du rapport de qualité     ║")
    print("[INFO] ╚═══════════════════════════════════════╝")
    ...
    try:
        ...
        ns_mail_series = ns_df[vcol].fillna("").astype(str).str.strip().str.lower() if vcol else ns_df.apply(lambda r: "", axis=1)
        ns_kpep_series = ns_df[kpep_col].fillna("").astype(str).str.strip() if kpep_col else ns_df.apply(lambda r: "", axis=1)
    ...
    if not args_ns.no_html:
        ...
        info("Rapport HTML non généré (--no-html).")
    info("Pipeline terminé avec succès !")

    # Console: résumé
    print("\n[INFO] === Résumé des KPI (pipeline) ===")
    k = payload["kpi"]
    print(f"       Assurés (N): {k['assures_n']}")
    print(f"       Assurés%: {k['assures_pct']}")
    print(f"       Radiés: {k['radies_n']} ({k['radies_pct']})")
    print(f"       E-mails valides: {k['emails_valid']}/{k['emails_non_empty']} ({k['emails_valid_pct']})")
    print(f"       Présence e-mail NS→CIAM: {k['ciam_email_presence_n']}/{k['ciam_email_total']} ({k['ciam_email_presence_pct']})")
    print(f"       Présence idrpp NS→IEHE: {k['iehe_presence_n']}/{k['iehe_total']} ({k['iehe_presence_pct']})")
    print(f"       Doublons: {k['dup_pct']} ({k['dup_n']} enregistrements)")
    print(f"       TP conformes: {k['tp_ok']}/{k['tp_expected']} ({k['tp_ok_pct']})")
    print(f"       TP futurs: {k['tp_future']}/{k['tp_expected']} ({k['tp_future_pct']})")
    print(f"       TP négatifs: {k['tp_negative']}/{k['tp_expected']} ({k['tp_negative_pct']})")
    try:
        print(f"       TP TOTAL (conformes + négatifs): {k.get('tp_total', '')}/{k.get('tp_expected', '')} ({k.get('tp_total_pct', '')})")
    except Exception:
        pass
    if args_ns.console_verbose:
        print("\n[INFO] === Détail complet des KPI ===")
        for key in sorted(k.keys()):
            print(f" - {key}: {k[key]}")
        if payload.get("exports"):
            print("\n[INFO] === Exports CSV générés ===")
            for e in payload["exports"]:
                try:
                    print(f"  - {e.get('label')} ({e.get('rows')} lignes) → {e.get('href')}")
                except Exception:
                    pass
